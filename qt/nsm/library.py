...
class Library:
    ...
    def _make(self, path, entry):
        ...
        # Размер сейва движок читает из поля `+4`. Фрейм здесь свой -
        # в источнике лежат только имя и блоки, - поэтому поле надо
        # заполнить: без него **всё считалось одноблочным**, и корзина
        # отводила десятиблочному Diablo один слот.
